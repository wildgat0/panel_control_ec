...
import { Square, Play, RotateCw, ScrollText, ChevronUp, Box } from 'lucide-react'
...
export default function ContainerCard({ container, onAction }) {
  ...
  return (
    <div className="rounded-xl flex flex-col gap-0 overflow-hidden transition-all"
      ...
      <div className="px-4 pt-4 pb-3 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3 text-xs" style={{ color: '#C4C87A' }}>
          ...
          {ports && (
            <span className="px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(196,200,122,0.08)', border: '1px solid #1A3A1A' }}>
              :{ports}
            </span>
          )}
        </div>
      ...
      <div className="px-4 py-3 flex items-center gap-2 flex-wrap" style={{ borderTop: '1px solid #1A3A1A' }}>
        ...
        <button onClick={toggleLogs}
          ...
          onMouseEnter={(e) => e.currentTarget.style.borderColor = '#2A5A2A'}
          onMouseLeave={(e) => e.currentTarget.style.borderColor = '#1A3A1A'}>
          {showLogs ? <ChevronUp size={11} /> : <ScrollText size={11} />}
      ...
      {showLogs && logs && (
        <div style={{ borderTop: '1px solid #1A3A1A' }}>
          <pre className="text-xs p-4 overflow-x-auto max-h-52 overflow-y-auto whitespace-pre-wrap"
            style={{ color: '#C4C87A', backgroundColor: '#050F05', fontFamily: 'monospace' }}>
            {logs}
          </pre>
        </div>
      )}
